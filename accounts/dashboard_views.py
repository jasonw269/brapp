...
def dashboard(request):
    ...
    if user.is_member:
        ...
        # Recent badges
        recent_badges = MemberBadge.objects.filter(user=user).select_related('badge').order_by('-awarded_at')[:5]
        context['recent_badges'] = recent_badges

        # Check-in events (events today or past that user is attending and hasn't checked in)
